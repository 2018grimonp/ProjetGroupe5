...
def creation_string(code):
    ...
    graph="Fonction copié-donne le nombre de fonctions suceptible d'e^tre fraudé (avec une précision minimale de "+str(precision)+"% -fonction originale+fonctions copiées-"+str(100-copies_percent)+"-Multiplicateur de note: "+str(copies_percent/10)+"/10 pourcentage de duplication :"+str(donnees[2])+"-|"
    return (graph)
