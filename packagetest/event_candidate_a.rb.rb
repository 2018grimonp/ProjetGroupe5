  ...
  def appointment?
    kind.eql? 'appointment' #Voil� on a des commentaires un peu partout #bonjour
  end
...
#Ok on va �crire un commentaire ici
...
=end
    events.each do |event|
      (event.starts_at.to_i..(event.ends_at.to_i - 30.minutes)).step(30.minutes) do |timestamp|
        slots << Time.at(timestamp).utc.strftime('%-H:%M') #et puis un autre l�
      end
    end
